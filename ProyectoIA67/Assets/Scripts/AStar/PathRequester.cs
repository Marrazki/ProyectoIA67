...
public class PathRequester : MonoBehaviour
{
    ...
    int        _pathIndex;
    bool       _isFollowing;
    AStarPathfinder.HeuristicType _lastHeuristicType;
    ...
    void Update()
    {
        ...
        // Recalcular si la heurística cambió en tiempo real
        if (AStarPathfinder.instance != null &&
            AStarPathfinder.instance.heuristicType != _lastHeuristicType)
        {
            RequestPath();
        }
    ...
    void RequestPath()
    {
        ...
        _lastHeuristicType = AStarPathfinder.instance.heuristicType;

        _isFollowing = false;

        // Cancelar coroutine anterior si estaba en curso
        if (_pathCoroutine != null)
        {
            StopCoroutine(_pathCoroutine);
            AStarPathfinder.instance.ClearVizState();
            _pathCoroutine = null;
        }

        if (AStarPathfinder.instance.stepByStep)
        {
            _pathCoroutine = StartCoroutine(
                AStarPathfinder.instance.FindPathAsync(
                    transform.position, target.position, OnPathFound));
        }
        else
        {
            OnPathFound(AStarPathfinder.instance.FindPath(
                transform.position, target.position));
        }
    }

    void OnPathFound(System.Collections.Generic.List<Node> path)
    {
        _currentPath = path;
        _pathIndex   = 0;
        _isFollowing = false;
