...
public class Enemigo_Steering_BT : MonoBehaviour
{
    ...
    void BuildTree()
    {
        _tree = new Selector(
            new ConditionalSequence(
                ...
                new BTAction(ApplyPursue, "Pursue-Jugador")
            )
            { Name = "Perseguir con steering [Self Abort]" },
    ...
    bool EstaCerca()
    {
        return Vector3.Distance(transform.position, jugador.position) < attackRange;
    }
    ...
    NodeStatus ApplySeek()
    {
        GetComponent<Renderer>().material.color = Color.yellow;
        Vector3 desired = SteeringBehaviors.Seek(
            transform.position, jugador.position, maxSpeed);
        _velocity = Vector3.MoveTowards(_velocity, desired, steeringForce * Time.deltaTime);
        return NodeStatus.Running;
    }

    NodeStatus ApplyPursue()
    {
        GetComponent<Renderer>().material.color = Color.yellow;
        Vector3 desired = SteeringBehaviors.Pursue(
            transform.position, jugador.position, jugador.GetComponent<Rigidbody>().linearVelocity, maxSpeed);
        _velocity = Vector3.MoveTowards(_velocity, desired, steeringForce * Time.deltaTime);
        return NodeStatus.Running;
    }
