...
{
    public enum SteeringMode { Seek, Flee, Arrive, Wander, FleeAndArrive }
    ...
    [Header("Wander")]
    public float wanderCircleDistance = 5f;
    public float wanderCircleRadius = 3f;
    public float wanderAngle = 15f;

    [Header("Flee and arrive")]
    ...
    Vector3 ComputeDesiredVelocity() => mode switch
    {
        ...
        SteeringMode.Wander => SteeringBehaviors.Wander(transform.position, transform.forward, 
                                                     ref wanderAngle, maxSpeed,
                                                     wanderCircleDistance, wanderCircleRadius),
        SteeringMode.FleeAndArrive => SteeringBehaviors.FleeAndArrive(
                                                        transform.position, 
                                                        threat.position,      
                                                        evacuationPoint.position,
                                                        maxSpeed, 
                                                        slowRadius, 
                                                        targetRadius),
        _ => Vector3.zero
