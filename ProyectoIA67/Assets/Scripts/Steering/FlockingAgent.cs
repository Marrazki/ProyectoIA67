...
public class FlockingAgent : MonoBehaviour
{
    ...
    public float avoidanceDetectionDistance = 5f;
    ...
    void Update()
    {
        ...
        Vector3 steering = cohesion + separation + alignment + avoidance;
    ...
    {
        Vector3 direction = _velocity.sqrMagnitude > 0.001f
            ? _velocity.normalized
            : transform.forward;

        // SphereCast desde la posición actual en la dirección de movimiento
        if (Physics.SphereCast(
            transform.position,
            avoidanceSphereRadius,
            direction,
            out RaycastHit hit,
            avoidanceDetectionDistance,
            obstacleLayer))
        {
            // Fuerza perpendicular al normal del impacto
            Vector3 avoidanceForce = Vector3.Cross(hit.normal, Vector3.up).normalized;

            // Amplificar según distancia (más cerca = más fuerte)
            float distanceFactor = 1f - (hit.distance / avoidanceDetectionDistance);

            return avoidanceForce * distanceFactor;
        }
